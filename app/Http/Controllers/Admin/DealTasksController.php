...
class DealTasksController extends Controller
{
    ...
    public function getRecords($id,Request $request)
    {
        ...
        try {
            ...
            // Total records
            $totalRecords = DealTask::select('count(*) as allcount')->where('deal_id',$broker->id)->whereNull('deleted_at')
                                                                                      ->count();
            ...
            // Fetch records
            $sql = DealTask::select(DB::raw('deal_tasks.id,deal_tasks.deal_id,processors.name as processor,deal_tasks.details,CONCAT(users_u.fname," ",users_u.lname) as user,CONCAT(users.fname," ",users.lname) as created_by,CONCAT(user_mod.fname," ",user_mod.lname) as modified_by,DATE_FORMAT(deal_tasks.created_at,"'.$this->mysql_date_format.' %H:%i:%s") as formated_created_at, DATE_FORMAT(deal_tasks.updated_at,"'.$this->mysql_date_format.' %H:%i:%s") as formated_updated_at,
            date_format(deal_tasks.followup_date,"'.$this->mysql_date_format.'") as  followup_date,date_format(deal_tasks.end_date,"'.$this->mysql_date_format.'") as  end_date, ( CASE deal_tasks.followup_detail WHEN 1 THEN "12 month follow up" WHEN 2 THEN "24 month follow up" END ) as followup_detail'))->leftJoin('processors','processors.id','=','deal_tasks.processor')->leftJoin('users as users_u','users_u.id','=','deal_tasks.user')->leftJoin('users','users.id','=','deal_tasks.created_by')->leftJoin('users as user_mod','user_mod.id','=','deal_tasks.updated_by')->orderBy(
                $columnName,
                $columnSortOrder
            )->where('deal_tasks.deal_id',$broker->id)->whereNull('deal_tasks.deleted_at')
                ->skip($start)
                ->take($rowperpage);
