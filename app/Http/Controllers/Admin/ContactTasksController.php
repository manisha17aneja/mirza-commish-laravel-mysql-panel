...
class ContactTasksController extends Controller
{
    public function brokerExpList($id) {
        ...
        $broker  = ContactSearch::select(DB::raw('id,CONCAT_WS(",",trading,trust_name,CONCAT(surname," ",first_name," ",middle_name),preferred_name,entity_name) as display_name,trading,trust_name,surname,first_name,middle_name,preferred_name,entity_name')
        )->find(decrypt($id));
    ...
    public function getRecords($id,Request $request)
    {
        ...
        try {
            ...
            // Fetch records
            $sql = TaskRelation::select(DB::raw('client_tasks.id,client_tasks.client_id,IF(client_tasks.followup_date IS NOT NULL,DATE_FORMAT(client_tasks.followup_date,"'.$this->mysql_date_format.'"),"") as followup_date,date_format(client_tasks.end_date,"'.$this->mysql_date_format.'") as  end_date,processors.name as processor,client_tasks.details,CONCAT(users.fname," ",users.lname) as created_by,CONCAT(users_u.fname," ",users_u.lname) as user,CONCAT(user_mod.fname," ",user_mod.lname) as modified_by,DATE_FORMAT(client_tasks.created_at,"'.$this->mysql_date_format.' %H:%i:%s") as formated_created_at, DATE_FORMAT(client_tasks.updated_at,"'.$this->mysql_date_format.' %H:%i:%s") as formated_updated_at'))->leftJoin('processors','processors.id','=','client_tasks.processor')->leftJoin('users as users_u','users_u.id','=','client_tasks.user')->leftJoin('users','users.id','=','client_tasks.created_by')->leftJoin('users as user_mod','user_mod.id','=','client_tasks.updated_by')->orderBy(
                $columnName,
                $columnSortOrder
    ...
    public function brokerEdit($id, $bid) {
        ...
        $data['contact'] = $broker;
        $data['processors'] = Processor::all();
        $data['taskdata'] = TaskRelation::select(DB::raw('client_tasks.*,IF(client_tasks.followup_date IS NOT NULL,DATE_FORMAT(client_tasks.followup_date,"'.$this->mysql_date_format.'"),"") as followup_date,date_format(end_date,"'.$this->mysql_date_format.'") as  end_date'))->where('client_id',$broker->id)->where('id','=',
            decrypt($id))
                                                                                 ->first();
