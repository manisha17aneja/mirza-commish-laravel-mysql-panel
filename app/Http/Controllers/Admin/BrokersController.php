...
class BrokersController extends Controller
{
    ...
    public function brokerAdd()
    {
        ...
        $data['refferors'] = ContactSearch::select(
            DB::raw('id,CONCAT(first_name," ",middle_name, " ",surname) as display_name,trading,trust_name,surname,first_name,middle_name,preferred_name,entity_name')
    ...
    public function brokerPost(Request $request)
    {
        $validated = $request->validate([
            ...
            'trading' => 'required|max:255|exclude_if:individual,1',
        ...
        try {
            ...
            $contact->type = $request['search_for'];
            $contact->is_individual = isset($request['individual']) && $request['individual'] ?  1 : 0;
    ...
    public function brokerEdit($id)
    {
        ...
        $broker = Broker::select(DB::raw('brokers.*,DATE_FORMAT(brokers.dob,"'.$this->mysql_date_format.'") as dob,DATE_FORMAT(brokers.start_date,"'.$this->mysql_date_format.'") as start_date,DATE_FORMAT(brokers.end_date,"'.$this->mysql_date_format.'") as end_date'))
                        ->with(['referrors'])->where('id', decrypt($id))->first();
    ...
    public function brokerView($id)
    {
        ...
        $frequencies = [
            ...
            '2' => 'Month',
             ...
             //->leftJoin('broker_commission_model as bcm','bcm.broker_id','=',$id)
            $institutes= Lenders::all(); 
            //join('broker_commission_model_institution as bcmi','bcmi.broker_id','=',decrypt($id))->get();
            //print_R($institutes);die;
        $commission_models = Commission::all();
            return view('admin.broker.view', compact('broker','exptypes','statuses','frequencies','fee_types','certifications','institutes','commission_models'));
        } else {
