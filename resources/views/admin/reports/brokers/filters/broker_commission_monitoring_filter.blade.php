<form method="POST" id="commission_monitoring_form" target="_blank">
    @csrf
             ...
             <div class="form-group">
                 ...
                 <select class="form-control" name="broker_id" id="broker_id">
                     ...
                     @foreach($brokers as $broker)
                         <option value="{{ $broker->id }}">{{ $broker->surname }}</option>
                     @endforeach
                 </select>
             </div>
         </div>
...
@section('scripts')
    ...
    <script>
        ...
        $(".preview").on("click",function (){
            $("#commission_monitoring_form").attr("action","{{ route("admin.broker.preview_commission_monitoring_report") }}");
            $("#commission_monitoring_form").submit();
        });
        $(".export_pdf").on("click",function (){
            $("#commission_monitoring_form").attr("action","{{ route("admin.broker.export_commission_monitoring_report") }}");
            $("#commission_monitoring_form").submit();
        });
    </script>
