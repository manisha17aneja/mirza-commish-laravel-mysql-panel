...
<table style="margin-top: 5px;margin-bottom:5px;width: 100%">
    <tbody>
    <tr>
        <td style="width: 30%"> <span style="font-size: 18px;font-weight: bold;">FM Direct Commission Outstanding</span></td>
    ...
    @foreach($deals as $deal)
        ...
        <table style="width: 100%;margin-top: 5px" >
            ...
            @foreach($deals_list as $k=>$deal_list)
                ...
                $trail=\App\Models\DealCommission::whereDealId($deal->id)->where('type',12)->sum('broker_amount');
                ?>
                <tr>
                    <td>{{ $deal_list->id }}</td>
                    ...
                    <td>${{$trail??'0' }}</td>
                    <td>${{ $t_tra_dif=$deal_list->broker_est_trail-$trail }}</td>
                </tr>
                <?php
                ...
                $actual_trail+=$trail;
                $est_trail+=$deal_list->broker_est_trail;
                $tra_dif+=$t_tra_dif;
                ?>
            @endforeach
            </tbody>
            ...
            <tr>
                ...
                <th style="text-align: left;width: 7%">${{ $est_trail }}</th>
                <th style="text-align: left;width: 7%">${{ $actual_trail }}</th>
                <th style="text-align: left;width: 7%">${{ $tra_dif }}</th>
            </tr>
            <?php
            ...
            $total_est_trail+=$est_trail;
            $total_actual_trail+=$actual_trail;
            $total_tra_dif+=$tra_dif;
            ?>
            </thead>
        </table>
    @endforeach
...
@if(count($deals)>0)
    ...
    <table class="grand_total"  style="width: 100%">
        <tr>
            ...
            <th style="text-align: left;width: 7%">${{ $total_est_trail }}</th>
            <th style="text-align: left;width: 7%">${{ $total_actual_trail }}</th>
            <th style="text-align: left;width: 7%">${{ $total_tra_dif }}</th>
        </tr>
    </table>
